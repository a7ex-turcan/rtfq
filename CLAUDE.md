...
**Discovery must survive an unreachable source.** Introspected schema is cached, and `describe_*` serves it — flagged with its age — even when the live source is down. Learning a table's shape must not require the database to be reachable at that instant; an agent should be able to draft a correct statement offline and only need the source live to run it. (This is not a nice-to-have; it is the single most useful property in practice.) Cache invalidation is TTL plus explicit `rtfq refresh <source>`; staleness is always visible in the response, never silent. **A miss against a *stale* cache is confirmed against the live source before it is reported absent, never asserted from the snapshot** — a table absent from a 12-day-old cache may simply be younger than it, and answering "not found" from stale metadata turns a completed migration into a phantom missing one. When the source is unreachable the answer says the object may exist and could not be confirmed, rather than claiming it is gone. `refresh` is exposed on every surface it is mentioned on — CLI, HTTP, and MCP tool — because an error that tells an agent to refresh while no such tool exists is the API lying about what it can do.
